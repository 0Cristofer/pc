...
__attribute__((transaction_safe)) leafptr InitLeaf(cellptr parent, unsigned ProcessId){
	leafptr l;
	int i, Mycell;
...

/* * INTCOORD: compute integerized coordinates.  * Returns: TRUE unless rp was out of bounds.  */
...
hackcofm(int nc,unsigned ProcessId){
	int i,Myindex;
	nodeptr r;
	leafptr l;
	leafptr* ll;
...

__attribute__((transaction_safe)) cellptr SubdivideLeaf (leafptr le, cellptr parent, unsigned int l, unsigned int ProcessId){
	cellptr c;
	int i, index;
	int xp[NDIM];
...
__attribute__((transaction_safe)) cellptr makecell(unsigned ProcessId){
	cellptr c;
	int i, Mycell;
