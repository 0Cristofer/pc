...
void output (unsigned int ProcessId);
...
void testdata(){
   real rsc, vsc, sqrt(double), xrand(double, double), pow(double, double), rsq, r, v, x, y;
   vector cmr, cmv;
   register bodyptr p;
   int rejects = 0;
   int k;
...
void pickshell(real vec[], real rad){
  register int k;
  double rsq, xrand(double, double), sqrt(double), rsc;
